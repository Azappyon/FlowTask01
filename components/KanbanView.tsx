...
import { PageHead, PlusIcon } from "./PageHead";
...
export function KanbanView() {
  ...
  async function quickAdd(status: string) {
    ...
    if (mode === "tarefas") {
      await saveTask({ title, status: status as any, priority: "media" });
    } else {
      await saveContent({ title, status: status as any });
    }
    setText("");
    setAdding(null);
  }
  ...
  return (
    <>
      <PageHead
        title="Kanban"
        subtitle="Arraste os cartões entre as colunas para mudar o status. Use “+ Adicionar” em qualquer coluna para criar."
        action={
          <div className="flex flex-wrap items-center gap-2">
            ...
            <button
              ...
              onClick={() => { setText(""); setAdding(cols[0][0]); }}
            >
              <PlusIcon /> {mode === "tarefas" ? "Nova tarefa" : "Novo conteúdo"}
            </button>
          </div>
        }
      />
      <div className="flex gap-3 overflow-x-auto pb-4 scrollbar-thin">
        {cols.map(([status, lbl]) => {
          ...
          return (
            <div key={status} className="flex w-64 flex-none flex-col rounded-xl p-2" style={{ background: "var(--bg)", border: "1px solid var(--line)" }}
              ...
              <div className="space-y-2">
                {items.map((it) => {
                  if (mode === "tarefas") {
                    ...
                    return (
                      <div key={t.id} draggable onDragStart={(e) => e.dataTransfer.setData("text/plain", t.id)} className="card cursor-grab p-2.5 transition active:scale-[0.98]">
                  ...
                  return (
                    <div key={c.id} draggable onDragStart={(e) => e.dataTransfer.setData("text/plain", c.id)} className="card cursor-grab p-2.5 transition active:scale-[0.98]">
                ...
                {adding === status ? (
                  <div className="card p-2">
                    <input
                      ...
                      placeholder={mode === "tarefas" ? "Título da tarefa…" : "Título do conteúdo…"}
                      value={text}
                      onChange={(e) => setText(e.target.value)}
                      onKeyDown={(e) => { if (e.key === "Enter") quickAdd(status); if (e.key === "Escape") { setAdding(null); setText(""); } }}
                    />
                    <div className="flex gap-2">
                      <button className="btn btn-primary flex-1 py-1.5" onClick={() => quickAdd(status)}>Adicionar</button>
                      <button className="btn btn-ghost py-1.5" onClick={() => { setAdding(null); setText(""); }}>Cancelar</button>
                    </div>
                  </div>
                ) : (
                  <button
                    onClick={() => { setText(""); setAdding(status); }}
                    className="w-full rounded-lg border border-dashed py-2 text-sm font-medium transition hover:bg-[var(--accent-soft)]"
